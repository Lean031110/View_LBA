...
test.describe("Temas — gestor completo (§22)", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByText("Tema activo:").locator("visible=true").first()).toBeVisible({ timeout: 20_000 })
  })
  ...
  test("1-5: importar Classic (.vtheme) → validado, instalado y aplicado", async ({ page }) => {
    ...
    await expect(page.getByText("Tema importado")).toBeVisible({ timeout: 15_000 })
    await expect(page.getByText("ViewLBA Classic E2E")).toBeVisible({ timeout: 10_000 })

    // 5. aplicar
    await page.locator(".tv-card", { hasText: "ViewLBA Classic E2E" }).getByRole("button", { name: "Aplicar" }).click()
    await expect(page.getByText("Tema aplicado")).toBeVisible({ timeout: 15_000 })

    // 6. comprobar estado: el card pasa a «Aplicado» y el banner de activo
    await expect(
      page.locator(".tv-card", { hasText: "ViewLBA Classic E2E" }).getByRole("button", { name: "Aplicado" })
  ...
  test("7-8: importar Neon y aplicar (cambio de tema)", async ({ page }) => {
    ...
    await expect(page.getByText("ViewLBA Neon E2E")).toBeVisible({ timeout: 10_000 })
    await page.locator(".tv-card", { hasText: "ViewLBA Neon E2E" }).getByRole("button", { name: "Aplicar" }).click()
    await expect(page.getByText("Tema aplicado")).toBeVisible({ timeout: 15_000 })
  ...
  test("12-13: eliminar Neon → Default permanece funcional", async ({ page }) => {
    ...
    await page.locator(".tv-card", { hasText: "ViewLBA Neon E2E" }).getByRole("button", { name: "Eliminar tema" }).click()
    await expect(page.getByText("Tema eliminado")).toBeVisible({ timeout: 15_000 })
    await expect(page.getByText("ViewLBA Neon E2E")).toHaveCount(0)

    // 13. el Default NO tiene botón de eliminar y se puede aplicar
    const defaultCard = page.locator(".tv-card", { hasText: "ViewLBA Default" })
  ...
  test("paquete MALICIOSO (assets/evil.js) → rechazo humano, sin crash", async ({ page }) => {
    ...
    await expect(page.getByText("No se pudo importar el tema")).toBeVisible({ timeout: 15_000 })
  ...
  test("tema integrado Classic: aplicar SIN importar (viene de fábrica)", async ({ page }) => {
    const card = page.locator(".tv-card", { hasText: "ViewLBA Classic" }).first()
