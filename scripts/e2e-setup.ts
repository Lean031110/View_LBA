 ...
 *  · LICENCIA E2E: activa una licencia firmada con la clave DUMMY de test
 *    (e2e/fixtures/licensing) vía POST /api/license/activate — así los specs
 *    existentes (crear pantallas, usuarios, settings) corren como ACTIVO sin
 *    verse afectados por el gating premium del trial. El trial/gating se
 *    prueba en tests unitarios y de integración dedicados (tests/licensing,
 *    tests/integration).
 *
...
// 5) LICENCIA E2E: activar token firmado con clave DUMMY de test.
...
const { activateLicenseToken } = await import("../src/lib/licensing/index")
const { buildE2eLicenseToken } = await import("../e2e/fixtures/licensing/license-builder")
const token = buildE2eLicenseToken({
  customerName: "Restaurante E2E",
  plan: "annual",
  durationDays: 365,
  licenseId: "VLBA-e2e000000001",
})
const activation = await activateLicenseToken(token, {
  actor: { uid: "e2e-setup", name: "E2E Setup" },
  publicKey: E2E_LICENSE_PUBLIC_KEY,
})
if (!activation.ok) {
  console.error("✗ No se pudo activar la licencia E2E:", activation.code, activation.reason)
  process.exit(1)
}
console.log(`✓ Licencia E2E activada (VLBA-e2e000000001 · vence en ${activation.summary?.daysLeft} días)`)
