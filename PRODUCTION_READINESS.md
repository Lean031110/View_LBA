...
| **Licenciamiento (v2.0.0)** | **Token copiar/pegar, offline 100%**: código de solicitud `VLREQ2-…` sellado (X25519 efímero→HKDF→AES-256-GCM; identidad del equipo OCULTA y cifrada) + token `VLBA2-…` firmado Ed25519 (plan/duración exactos, binding, features, nonce) · activación por pipeline de 12 pasos con firma + binding recalculado contra hardware (clone = MISMATCH) + anti-replay idempotente + anti-downgrade · trial 7 días con anclas duales + anti-rollback de reloj · watermark TV · planes monthly 30/USD 10, annual 365/USD 100, custom 1–3650 · feature gating server-side · auditoría · **app Android privada del emisor** (SQLCipher + Keystore + PIN/biometría + auto-lock, backup .vlbak autenticado, anti-replay/downgrade, renovación desde historial) · CI Android con firma por secrets y análisis anti-claves del APK · las claves privadas NUNCA en el repo (gitleaks + vectores dorados TS↔Kotlin en CI) |
...
20260911000000_license_state       (LicenseState/LicenseHistory v1 — RETIRADA)
20260912000000_license_v2_token     (LicenseState{token,payloadJson} + LicenseHistory v2;
                                      recreación deliberada: cero licencias v1 emitidas)
```
